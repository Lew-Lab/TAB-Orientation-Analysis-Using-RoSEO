...
brightnessScaled_th = 1.3 * mean(b(:));
...
brightnessScaled_xx = max(gamma_init_t_xx(:));

gamma_init_t_yy = real(ifft2(bsxfun(@times, conj(FYYx), fft2(SMLM_img_r_x)))) + ...
    real(ifft2(bsxfun(@times, conj(FYYy), fft2(SMLM_img_r_y))));
brightnessScaled_yy = max(gamma_init_t_yy(:));

gamma_init_t_zz = real(ifft2(bsxfun(@times, conj(FZZx), fft2(SMLM_img_r_x)))) + ...
    real(ifft2(bsxfun(@times, conj(FZZy), fft2(SMLM_img_r_y))));

brightnessScaled_zz = max(gamma_init_t_zz(:));

if isfield(s, 'brightnessscaledfilter') && s.brightnessscaledfilter && ...
        max([brightnessScaled_yy, brightnessScaled_xx, brightnessScaled_zz]) < brightnessScaled_th
    gammaf = [];
    loc_data = [];
    return
...
l = l_max / max(SMLM_img(:)); % emprically, this initialization results in fast convergence
...
tmp = get_loc_data2(gammhat2, recovStruct);
EdgeGap = 20; % for trispot
if isfield(s, 'edgefilter') && ~any((tmp(:, 2) < (img_size / 2 - EdgeGap)*pixel_size).*(tmp(:, 2) > -(img_size / 2 - EdgeGap) * pixel_size).* ...
        (tmp(:, 3) < (img_size / 2 - EdgeGap) * pixel_size).*(tmp(:, 3) > -(img_size / 2 - EdgeGap) * pixel_size))
    gammaf = [];
    loc_data = [];
    return
end
...
if isfield(s, 'detection') && s.detection
...
for nn = 1:n_sub_frame + 1
    ...
    while (i < MaxIt2)
        ...
        % refine background estimates
        %----------------------------
        % step 1=> rearange
        if mod(i, 40) == 0
            molPhotons = fA(z);
            backg_tmp = reshape(SMLM_it-molPhotons, img_size, 2*img_size);
            b_it_tmp = reshape(b_it, img_size, 2*img_size);
            % step 2=> remove molecular photons
            b_it_offset_x = backg_tmp(:, 1:img_size, :) - b_it_tmp(:, 1:img_size, :);
            b_it_offset_x = mean(b_it_offset_x(:));
            b_it_offset_y = backg_tmp(:, 1+img_size:end, :) - b_it_tmp(:, 1+img_size:end, :);
            b_it_offset_y = mean(b_it_offset_y(:));
            % update
            b_it_tmp(:, 1:img_size, :) = b_it_tmp(:, 1:img_size, :) + b_it_offset_x;
            b_it_tmp(:, 1+img_size:end, :) = b_it_tmp(:, 1+img_size:end, :) + b_it_offset_y;
            b_it = reshape(b_it_tmp, 2*img_size^2, 1);

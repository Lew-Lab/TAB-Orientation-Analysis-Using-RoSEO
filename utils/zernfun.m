function z = zernfun(n, m, r, theta, c, nflag)
...
% Check normalization:
% --------------------
if nargin == 6 && ischar(nflag)
    isweighted = strcmpi(nflag, 'weighted');
    if ~isweighted
        error('zernfun:normalization', 'Unrecognized weight flag.')
    end
else
    isweighted = false;
end
...
if rpowers(1) == 0
    ...
    rpowern = cat(3, ones(size(r, 1), size(r, 1), 1), reshape(cell2mat(rpowern), size(r, 1), size(r, 2), numel(rpowern)));
    %     rpowern = [ones(size(r,1),size(r,1),1) rpowern];
else
    rpowern = arrayfun(@(p)r.^p, rpowers, 'UniformOutput', false);
    rpowern = reshape(cell2mat(rpowern), size(r, 1), size(r, 2), numel(rpowern));
end

% Compute the values of the polynomials:
% --------------------------------------
z = zeros(size(r, 1), size(r, 1), length(n));
for j = 1:length(n)
    s = 0:(n(j) - m_abs(j)) / 2;
    pows = n(j):-2:m_abs(j);
    for k = length(s):-1:1
...
if any(indx_pos)
    z(:, :, indx_pos) = z(:, :, indx_pos) .* cos(repmat(theta, 1, 1, sum(indx_pos > 0)).* ...
        repmat(reshape(m_abs(indx_pos), 1, 1, sum(indx_pos > 0)), size(theta, 1), size(theta, 1), 1));
end
if any(indx_neg)
    z(:, :, indx_neg) = z(:, :, indx_neg) .* sin(repmat(theta, 1, 1, sum(indx_neg > 0)).* ...
        repmat(reshape(m_abs(indx_neg), 1, 1, sum(indx_neg > 0)), size(theta, 1), size(theta, 1), 1));
end

...
% Image address
imageFile = ...
    'E:\DocuBak\TD_WZ TAB RoSEO\Tara Porter\190729_2158BBP laser irradiation tests\offsetSubtracted';
imageFilePrefix = 'Data'; % prefix of your data name
imageFileInd = [15 16]; % data index you want to analyze. it accepts multiple data set.
% File format
fFormat = '.tif';
...
for stackInd = 1:length(imageFileInd)
    ...
    for hSS = 1:totSubStack
        ...
        for h = ( (hSS-1)*aveRate+1 ):(hSS*aveRate)
            capturedImage = double(imread([imageFile filesep imageFilePrefix num2str(imageFileInd(stackInd)) filesep dataFolderInfo(h).name])); 
%             capturedImage = double(imread([imageFile filesep imageFilePrefix num2str(imageFileInd(stackInd)) filesep 'SM2NaN\' dataFolderInfo(h).name])); %WZ

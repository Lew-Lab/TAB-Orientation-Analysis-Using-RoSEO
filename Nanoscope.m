classdef Nanoscope
    ...
    properties
        ...
        emissWavelength = 593; % central emission wavelength (nm)
        ADcount = .29; % photoelectron conversion
        offset = 0; % baseline of camera (in digital units)
        refractiveIndx = 1.518; % refractive index of the medium
        sampleRefractiveIndx = 1.334; % refractive index of the sample itself
        imageSize = 101; % side length of the region of interest
        ...
        % phase mask parameters
        %--------------------------------------------------------
        phaseMaskPara = struct('maskName', 'flatMask_0_PupilRadius_80', ... %parameters of the phase mask mounted on SLM
            'pupilRadius', 80, ...
            'x_shift_center_position', 0, ...
            'y_shift_center_position', 0, ...
            'maskRotation', 0,...
            'zeroOrder',[0 0]);%210303
        ...
        %===========================
        %electric field at pupil plane
        %---------------------------------------------------------

        EXXxBasis %elctric field corresponding to XX (x_channel) at pupil plane of microscope
        EYYxBasis %elctric field corresponding to YY (x_channel) at pupil plane of microscope
        EZZxBasis %elctric field corresponding to ZZ (x_channel) at pupil plane of microscope
        EXXyBasis %elctric field corresponding to XX (y_channel) at pupil plane of microscope
        EYYyBasis %elctric field corresponding to YY (y_channel) at pupil plane of microscope
        EZZyBasis %elctric field corresponding to ZZ (y_channel) at pupil plane of microscope
        %===========================
        %calibration ad pupil recovery parameters
        %---------------------------------------------------------
        pupilRecovPara = struct('sizeROI', 35, ... %parameters of pupil recovery
            'centerROI', [], 'maxIndxZernikeMode', 7);
        ZernikeBases = []; % Zernike bases at pupil plane of microscope
        ZernikeBases_upsampled = [];
        ZernikeCoefficient = struct('x', [], 'y', []); % Zernike coefficients (structure with fields of arrays
        %with size= num_coefficients)
    end
    ...
    methods
        ...
        function obj = Nanoscope(varargin)
            ...
            if isfield(s, 'pixelupsample')
                if (isnumeric(s.pixelupsample) && s.pixelupsample > 0 && s.pixelupsample == floor(s.pixelupsample))
                    obj.pixelUpsample = s.pixelupsample;
                else
                    msg = sprintf(['Expecting a positive integer type for pixelupsample.\n', ...
                        'Setting pixelupsample property to default %.2f'], obj.pixelUpsample);
                    warning('Nanoscope:InconsistentInputType', msg)
                end
            end
            ...
            if isfield(s, 'samplerefractiveindx')
                if (isnumeric(s.samplerefractiveindx) && s.samplerefractiveindx > 0)
                    obj.sampleRefractiveIndx = s.samplerefractiveindx;
                else
                    msg = sprintf(['Expecting a positive numeric type for sampleRefractiveIndx.\n', ...
                        'Setting sampleRefractiveIndx property to default %.2f'], obj.sampleRefractiveIndx);
                    warning('Nanoscope:InconsistentInputType', msg)
                end
            end
            ...
            if isfield(s, 'zernikecoefficient')
                if (isstruct(s.zernikecoefficient))
                    obj.ZernikeCoefficient = s.zernikecoefficient;
                else
                    msg = sprintf(['Expecting a structure for zernikecoefficient.\n', ...
                        'Setting zernikecoefficient property to default %.2f'], obj.ZernikeCoefficient);
                    warning('Nanoscope:InconsistentInputType', msg)
                end
            end
            ...
            ImagingInfo = sprintf(['pixel size: %.2f\n', 'numerical aperture: %.2f\n', ...
                ...
                obj.offset, obj.ADcount, obj.refractiveIndx, obj.sampleRefractiveIndx, obj.imageSize);
            ...
            disp(phaseMaskInfo)
            %=========================
            if isfield(s, 'pupilrecovpara')

                if isfield(s.pupilrecovpara, 'sizeroi')

                    obj.pupilRecovPara.sizeROI = s.pupilrecovpara.sizeroi;
                end
                if isfield(s.pupilrecovpara, 'centerroi')

                    obj.pupilRecovPara.centerROI = s.pupilrecovpara.centerroi;
                end


                if isfield(s.pupilrecovpara, 'maxindxzernikemode')
                    obj.pupilRecovPara.maxIndxZernikeMode = s.pupilrecovpara.maxindxzernikemode;
                end
            ...
            [~, ~, obj.ZernikeBases] = obj.computeBasis(obj, 'XX' ... % 210201
                , true, 'x_channel', true, 'crop', false, ...
                'ZernikeCompute', true);
            
            %brightness scaling           
            [scalingTempY,scalingTempX] = obj.brightnessScalingCompute();
            obj.brightnessScaling = scalingTempX+scalingTempY; 
            ...
            [~, obj.EXXxBasis] = obj.computeBasis(obj, 'XX' ... % 210201
                , true, 'x_channel', true, 'crop', false, ...
                'ZernikeCompute', true);
            [~, obj.EYYxBasis] = obj.computeBasis(obj, 'YY' ...
                , true, 'x_channel', true, 'crop', false);
            [~, obj.EZZxBasis] = obj.computeBasis(obj, 'ZZ' ...
                , true, 'x_channel', true, 'crop', false);
            ...
            [~, obj.EXXyBasis] = obj.computeBasis(obj, 'XX' ...
                , true, 'y_channel', true, 'crop', false);

            [~, obj.EYYyBasis] = obj.computeBasis(obj, 'YY' ...
                , true, 'y_channel', true, 'crop', false);

            [~, obj.EZZyBasis] = obj.computeBasis(obj, 'ZZ' ...
                , true, 'y_channel', true, 'crop', false);
        end
        ...
        function obj=set.sampleRefractiveIndx(obj,val)
            if nargin>0
                if ~isnumeric(val)
        ...
        function obj = set.phaseMaskPara(obj, s)
            ...
            if nargin > 0
                ...
                if isfield(s, 'zeroorder') % 210303

                    if ~((isnumeric(s.zeroorder(1)) && (size(s.zeroorder,2) == 2)))

                        error('Nanoscope:InconsistentInputType', 'Expecting two elements with values 0-1 for zeroorder')

                    end
                    s_t.zeroOrder = s.zeroorder;

                else
                    s_t.zeroOrder = obj.phaseMaskPara.zeroOrder;
                end
...
%         function obj = set.pupilRecovPara(obj, s)
% 
%             if nargin > 0
%                 s_t = struct();
% 
...
%                 end
% 
%                 if isfield(s, 'sizeroi')
% 
%                     if ~(ischar(s.sizeroi))
% 
%                         error('Nanoscope:InconsistentInputType', 'Expecting a character array for sizeROI')
% 
%                     end
%                     s_t.sizeROI = s.sizeROI;
%                 else
% 
%                     s_t.sizeROI = obj.pupilRecovPara.sizeROI;
%                 end
% 
%                 if isfield(s, 'centerroi')
% 
%                     if ~(isnumeric(s.centerroi))
% 
%                         error('Nanoscope:InconsistentInputType', 'Expecting a positive number for centerROI')
% 
%                     end
%                     s_t.centerROI = s.centerroi;
% 
%                 else
%                     s_t.centerROI = obj.pupilRecovPara.centerROI;
%                 end
% 
% 
%                 if isfield(s, 'maxindxzernikemode')
% 
%                     if ~(isnumeric(s.maxindxzernikemode))
% 
%                         error('Nanoscope:InconsistentInputType', 'Expecting a numeric type for maxindxzernikemode')
%                     end
%                     s_t.maxIndxZernikeMode = s.maxindxzernikemode;
%                 else
% 
%                     s_t.maxIndxZernikeMode = obj.pupilRecovPara.maxIndxZdernikeMode;
%                 end
% 
%             end
% 
%             obj.pupilRecovPara = struct(s_t);
%         end
    ...
    methods (Access = protected)
        ...
        function [brightnessScalingy, brightnessScalingx] = brightnessScalingCompute(obj)
            ...
            [brightnessScalingx, brightnessScalingy] = obj.simDipole_novotny(obj, Emitter);
        end

        %%

        function [B, E, Z] = computeBasis(obj, varargin)
            ...
            if isfield(s, 'xx') || isfield(s, 'xy') || isfield(s, 'xz')
                ...
                [BXXx, BXXy, EXXx, EXXy, phi_p, rho_p] = simDipole_novotny_h(Emitter);
            ...
            if isfield(s, 'yy') || isfield(s, 'xy') || isfield(s, 'yz')
                [BYYx, BYYy, EYYx, EYYy] = simDipole_novotny_h(Emitter);
            ...
            if isfield(s, 'zz') || isfield(s, 'xz') || isfield(s, 'yz')
                [BZZx, BZZy, EZZx, EZZy] = simDipole_novotny_h(Emitter);
            ...
            if isfield(s, 'zernikecompute') && s.zernikecompute

                %compute zernike bases
                nindx = [];
                mindx = [];

                %exclude first three modes
                for i = 1:obj.pupilRecovPara.maxIndxZernikeMode
                ...
                if ~isempty(mindx)
                    Z = zernfun(nindx, mindx, rho_p, phi_p);
    ...
    methods (Static)
        ...
        function phaseMask_out = mountPhaseMask(obj, varargin)
            ...
            maskSize = round((obj.pixelSize/obj.pixelUpsample)^-1*(obj.emissWavelength * rho_max)/obj.numApt);
            if mod(maskSize, 2) ~= 0
                maskSize = maskSize + 1;
            end
            ...
            scaleFactor = (rho_max+1) * 2 / mask_aper;
            ...
            %the initial amplitude was inverse of the true amplitude!
...
%             end % 210303
    ...
    methods (Static)
        ...
        function [imgx, imgy, Ex_rotated, Ey_flipped, phi_p, rho_p] = simDipole_novotny(Nanoscope, Emitter, varargin)
            ...
            n2 = Nanoscope.sampleRefractiveIndx; % sample refractive index
            ...
            if isfield(s, 'upsampling')
                ...
                upsampling = Nanoscope.pixelUpsample; %upsampling factor of image space
            end
            ...
            rp_2h = (nh*costheta2 - n2*costhetah)./(n2*costhetah+ nh*costheta2);
            rs_2h = (n2*costheta2 - nh*costhetah)./(n2*costheta2+ nh*costhetah);
            rp_h1 = (n1*costhetah - nh*costheta1)./(nh*costheta1+ n1*costhetah);
            rs_h1 = (nh*costhetah - n1*costheta1)./(nh*costhetah+ n1*costheta1);
...
%             rs_h1 = (n1 * costhetah - nh * costheta1) ./ (n1 * costhetah + nh * costheta1);
            ...
            if contains(Nanoscope.phaseMaskPara.maskName,'bifocal')
                Ex_1 = exp(1i*k1*(bsxfun(@times,reshape(z-200,1,1,molecule_num),costheta1))); %defocus term at pupil plane
                % shift 200 nm in x channel only for bifocal PSF                
            else                
            ...
            if contains(Nanoscope.phaseMaskPara.maskName,'raPol')
                J11 = cos(phi);
                J22 = cos(phi);
                J12 = sin(phi);
                J21 = -sin(phi);
                
                Ex_out = J11.*Ex + J12.*Ey;
                Ey_out = J21.*Ex + J22.*Ey;

                Ex = Ex_out;
                Ey = Ey_out;
            elseif contains(Nanoscope.phaseMaskPara.maskName,'chido') % CHIDO, DOI: 10.1038/s41467-020-19064-6
                chidoPara = pi; %1.2*pi;
                J11tmp = cos(chidoPara*(rho/rho_max)/2) + 1j*sin(chidoPara*(rho/rho_max)/2).*cos(phi);
                J12tmp = -1j*sin(chidoPara*(rho/rho_max)/2).*sin(phi);
                J21tmp = -1j*sin(chidoPara*(rho/rho_max)/2).*sin(phi);
                J22tmp = cos(chidoPara*(rho/rho_max)/2) - 1j*sin(chidoPara*(rho/rho_max)/2).*cos(phi);
%                 J = [J11tmp J12tmp;J21tmp J22tmp];
                ttheta = pi/4; % rotated quarter-wave plate with angle ttheta
                R_QR = [cos(ttheta)^2+1j*sin(ttheta)^2 (1-1j)*sin(ttheta)*cos(ttheta);...
                    (1-1j)*sin(ttheta)*cos(ttheta) sin(ttheta)^2+1j*cos(ttheta)^2]; 
                J11 = (R_QR(1,1)*J11tmp + R_QR(1,2)*J21tmp);
                J12 = (R_QR(1,1)*J12tmp + R_QR(1,2)*J22tmp);
                J21 = (R_QR(2,1)*J11tmp + R_QR(2,2)*J21tmp);
                J22 = (R_QR(2,1)*J12tmp + R_QR(2,2)*J22tmp);
                Ex_out = J11.*Ex + J12.*Ey;
                Ey_out = J21.*Ex + J22.*Ey;
            ...
            %apply aberration at pupil plane
            phaseAberr_x = ones(size(pmask_x));
            phaseAberr_y = ones(size(pmask_y));
            
            if isfield(Nanoscope.ZernikeCoefficient, 'x') && ~any(structfun(@isempty, Nanoscope.ZernikeCoefficient)) && ...
                    ~isempty(Nanoscope.ZernikeBases) % 210201

                c_x = Nanoscope.ZernikeCoefficient.x;
                c_y = Nanoscope.ZernikeCoefficient.y;
                phaseAberr_x = ones([size(pmask_x), size(c_x, 2)]);
                phaseAberr_y = ones([size(pmask_y), size(c_x, 2)]);

                if (size(c_x, 1) ~= size(Nanoscope.ZernikeBases, 3) || ~isnumeric(Nanoscope.ZernikeCoefficient.x))
                    error('Nanoscope2D:simDipole_novotny:InconsistentInputType', ...
                        'Expecting c*k numeric array for Zernike coefficients. c is the number of modes.')
                end
                if (~isnumeric(Nanoscope.ZernikeCoefficient.y))
                    error('Nanoscope2D:simDipole_novotny:InconsistentInputType', ...
                        'Expecting  numeric array for Zernike coefficients')
                end

                if ~(isempty(c_x) && isempty(c_y) || isempty(Nanoscope.ZernikeBases))

                    if upsampling > 1

                        for jj = 1:size(c_x, 2)
                            phaseAberr_x(:, :, jj) = exp(1i*sum(bsxfun(@times, Nanoscope.ZernikeBases_upsampled, ...
                                reshape(c_x(:, jj), 1, 1, max(size(c_X(:, jj))))), 3));
                        end
                        phaseAberr_x = mean(phaseAberr_x, 3);

                        for jj = 1:size(c_y, 2)
                            phaseAberr_y(:, :, jj) = exp(1i*sum(bsxfun(@times, Nanoscope.ZernikeBases_upsampled, ...
                                reshape(c_y(:, jj), 1, 1, max(size(c_y(:, jj))))), 3));
                        end
                        phaseAberr_y = mean(phaseAberr_y, 3);
                    else
                        for jj = 1:size(c_x, 2)
                            phaseAberr_x(:, :, jj) = exp(1i*sum(bsxfun(@times, Nanoscope.ZernikeBases, ...
                                reshape(c_x(:, jj), 1, 1, max(size(c_x(:, jj))))), 3));
                        end
                        phaseAberr_x = mean(phaseAberr_x, 3);
                        for jj = 1:size(c_y, 2)
                            phaseAberr_y(:, :, jj) = exp(1i*sum(bsxfun(@times, Nanoscope.ZernikeBases, ...
                                reshape(c_y(:, jj), 1, 1, max(size(c_y(:, jj))))), 3));
                        end
                        phaseAberr_y = mean(phaseAberr_y, 3);
                    end
                    ...
                    warning('Nanoscope2D:simDipole_novotny:InconsistentInputValue', ...
                        'Expecting non-empty Zernike coefficients and Zernike bases. Aberration is ignored. ')
                end
            end

            if isfield(s, 'zernikecoefficient') && ~any(structfun(@isempty, s.zernikecoefficient))

                c_x = s.zernikecoefficient.x;
                c_y = s.zernikecoefficient.y;
                phaseAberr_x = ones([size(pmask_x), size(c_x, 2)]);
                phaseAberr_y = ones([size(pmask_y), size(c_x, 2)]);

                if (size(c_x, 1) ~= size(Nanoscope.ZernikeBases, 3) || ~isnumeric(s.zernikecoefficient.x))
                    error('Nanoscope2D:simDipole_novotny:InconsistentInputType', ...
                        'Expecting c*k numeric array for Zernike coefficients. c is the number of modes.')
                end
                if (~isnumeric(s.zernikecoefficient.y))
                    error('Nanoscope2D:simDipole_novotny:InconsistentInputType', ...
                        'Expecting  numeric array for Zernike coefficients')
                end
            ...
            imgy = fftshift(fft2(ifftshift(Ey_flipped .* phaseAberr_y .* repmat(pmask_y, 1, 1, molecule_num))));
            imgx = fftshift(fft2(ifftshift(Ex_rotated .* phaseAberr_x .* repmat(pmask_x, 1, 1, molecule_num))));
            ...
            if isfield(s, 'gausssmoothsigmapixel') && s.gausssmoothsigmapixel > 0
                imgy = imgaussfilt(imgy, s.gausssmoothsigmapixel);
                imgx = imgaussfilt(imgx, s.gausssmoothsigmapixel);
            end
            
            if sum(Nanoscope.phaseMaskPara.zeroOrder) ~= 0 % 210303
                imgyFlat = fftshift(fft2(ifftshift(Ey_flipped .* phaseAberr_y .* repmat(abs(pmask_y), 1, 1, molecule_num))));
                imgxFlat = fftshift(fft2(ifftshift(Ex_rotated .* phaseAberr_x .* repmat(abs(pmask_x), 1, 1, molecule_num))));
                
                imgxFlat = permute(imgxFlat, [2, 1, 3]);
                imgyFlat = rot90(imgyFlat, 2);
                imgxFlat = rot90(imgxFlat, 2);
                % image on the camera is the amplitude squared of the electric field
                %--------------------------------------------------------
                imgyFlat = abs(imgyFlat).^2;
                imgxFlat = abs(imgxFlat).^2;
                
                if isfield(s, 'gausssmoothsigmapixel') && s.gausssmoothsigmapixel > 0
                    imgyFlat = imgaussfilt(imgyFlat, s.gausssmoothsigmapixel);
                    imgxFlat = imgaussfilt(imgxFlat, s.gausssmoothsigmapixel);
                end
                                
                imgy = (1-Nanoscope.phaseMaskPara.zeroOrder(2))*imgy + Nanoscope.phaseMaskPara.zeroOrder(2)*imgyFlat;
                imgx = (1-Nanoscope.phaseMaskPara.zeroOrder(1))*imgx + Nanoscope.phaseMaskPara.zeroOrder(1)*imgxFlat;                
            end
            
            %---------------------------------------------------------
            rho_p = squeeze(rho(:, :, 1)) ./ rho_max(1, 1, 1);
            phi_p = squeeze(phi(:, :, 1));
        end

        %% single channel version of basis generator, 210625
        function [img, Ex, Ey, phi_p, rho_p] = simDipole_novotny_sing(Nanoscope, Emitter, varargin)
            %simDipole_novotny computes
...
%             pmask_y = pmask(:, :, 2);
            ...
            imgx = fftshift(fft2(ifftshift(Ex .* repmat(pmask, 1, 1, molecule_num))));
...
%             rs_h1 = (n1 * costhetah - nh * costheta1) ./ (n1 * costhetah + nh * costheta1);
        ...
        %% single channel version of computeBases, 210625
        function [BXX, BYY, BZZ, BXY, BXZ, BYZ] = computeBases_sing(Nanoscope, Emitter, varargin)
            %computeBases computes

            num_molecules = numel(Emitter.position_para.x);
            %number of molecules

            simDipole_novotny_h = @(Emitter)Nanoscope.simDipole_novotny_sing(Nanoscope, Emitter, varargin{:});
            % XX
            ...
            [BXX] = simDipole_novotny_h(Emitter);
            ...
            [BYY] = simDipole_novotny_h(Emitter);
            ...
            [BZZ] = simDipole_novotny_h(Emitter);
            ...
            [BXYt] = simDipole_novotny_h(Emitter);

            % XZ
            Emitter.polar_para.phiD = ones(num_molecules, 1) * 0;
            Emitter.polar_para.thetaD = ones(num_molecules, 1) * pi / 4;

            [BXZt] = simDipole_novotny_h(Emitter);
            ...
            [BYZt] = simDipole_novotny_h(Emitter);
        ...
        function [FPSFx, FPSFy] = createPSFstruct(obj, varargin)
            ...
            if isfield(s, 'upsampling')
                ...
                [~, ~, obj.ZernikeBases_upsampled] = obj.computeBasis(obj, 'XX' ...
                    , true, 'x_channel', true, 'crop', false, ...
                    'ZernikeCompute', true, ...
                    'upsampling', up_sample);

            else
                up_sample = obj.pixelUpsample; % this is 1 by default
            end


            %determine Gaussian smoothing

            if isfield(s, 'gausssmoothsigmapixel')
                gaussSmoothSigma = s.gausssmoothsigmapixel * up_sample;
            else
                gaussSmoothSigma = 0;
            end
            ...
            %determine Zernike coefficients
            if isfield(s, 'zernikecoefficient')
                zernikeCoefficient = s.zernikecoefficient;
            else
                zernikeCoefficient = obj.ZernikeCoefficient;
            end
            ...
            %check if Zernike coefficients are supplied by the user; if
            %not set them to zero
            try
                zernikeCoefficient_t = zernikeCoefficient;

            catch ME
                if strcmp(ME.identifier, 'MATLAB:badsubscript')
                    zernikeCoefficient_t.x = 0;
                    zernikeCoefficient_t.y = 0;
                else
                    rethrow(ME)
                end

                if ~isstruct(zernikeCoefficient_t)
                    error('Expecting a structure for zernikeCoefficient.')
                end
            ...
            simDipole_novotny_h = @(Emitter) obj.simDipole_novotny(obj, ...
                Emitter, 'upsampling', up_sample, 'gausssmoothsigmapixel', ...
                gaussSmoothSigma, ...
                'zernikeCoefficient', zernikeCoefficient_t);
            ...
            [bscaley, bscalex] = obj.brightnessScalingCompute();
            sumNormalizex = sum(sum(roi(bscalex)));
            sumNormalizey = sum(sum(roi(bscaley)));
            sumNormalize = sumNormalizex + sumNormalizey;
            %x_channel
            %---------------------------------------------------------
    ...
    methods
        ...
        function [imgx, imgy] = formImage(obj, Emitter, varargin)
            ...
            if isfield(Emitter.position_para, 'z')
                zpos = Emitter.position_para.z; % in nm
            else
                zpos = Emitter.position_para.y * 0; % in nm
            end
            ...
            Emitter_t.position_para.z = zpos;
            ...
            [brightness_scalingx, brightness_scalingy] = obj.simDipole_novotny(obj, Emitter_t);
            ...
            sumnorm = sum(sum(roi(brightness_scalingx))) + sum(sum(roi(brightness_scalingy)));
            if isfield(s, 'ytoxchanneltransratio')
                ytoxchanneltransratio = s.ytoxchanneltransratio;
            else
                ytoxchanneltransratio = 1;
            end

            imgx = (imgx_corped) / sumnorm;
            imgy = (imgy_corped) / ((1 / ytoxchanneltransratio) * sumnorm);
    ...
    methods
        ...
        function [SMLM_img, folder_path, posRect, centerROI] = loadImgInteractive(obj, varargin)
            ...
            elseif isfield(s, 'offsetpath') && isempty(s.offsetpath)
            ...
            if isfield(s, 'tform')
                ...
                % read images
                SMLM_raw_img_2=nan(height,width,...
                number_frames,number_axial_scans);
    ...
    methods (Static)
        function SWResult = slidingWindow(obj, img, varargin)
            ...
            if isfield(s, 'buffer')
                ...
                buffer = 30; % size of pixel buffer between subimages. Ensures that PSFs are rarely cut off.
            end
            sizeROI = obj.imageSize;
            stepSize = sizeROI - buffer;

            if isfield(s, 'postoleft')
                posRect = s.postoleft;
            else
                posRect = [1, 1];
            end
            if (isfield(s, 'animate') && s.animate)
                imagesc(img(:, :, 1))
                axis image
            end

            %validate selected ROI
            %------------------------------------------------
            min_width = sizeROI;
            min_height = sizeROI;
            if isfield(s, 'width')
            ...
            if (width < min_width || height < min_height)
                error('Sorry! The selected ROI is too small')
            end


            %get the number of center positions.
            numxPosCenters = floor((width-sizeROI)/(sizeROI - buffer)) + 1;
            numyPosCenters = floor((height-sizeROI)/(sizeROI - buffer)) + 1;
            nWindows = numxPosCenters * numyPosCenters;
            SWResult = cell(1, nWindows); % for each window, outputs raw window data,
            %  center x,y position.

            windowIndex = 1;

            %loop over center positions
            for ncy = 1:numyPosCenters
                for ncx = 1:numxPosCenters
                    curCornerPos = [(ncx - 1) * stepSize + posRect(1), (ncy - 1) * stepSize + posRect(2)];
                    curxPosCenter = (ncx - 1) * (sizeROI - buffer) + (sizeROI - 1) / 2 + posRect(1);
                    curyPosCenter = (ncy - 1) * (sizeROI - buffer) + (sizeROI - 1) / 2 + posRect(2);
                    curHorizantalIndx = curxPosCenter - ((sizeROI - 1) / 2):curxPosCenter + ((sizeROI - 1) / 2);
                    curVerticalIndx = curyPosCenter - ((sizeROI - 1) / 2):curyPosCenter + ((sizeROI - 1) / 2);

                    if (isfield(s, 'animate') && s.animate)
                        hold on
                        rectangle('Position', [curCornerPos(1), curCornerPos(2), sizeROI, sizeROI], 'EdgeColor', 'r', ...
                            'LineStyle', '-', 'LineWidth', 2);
                        rectangle('Position', [curCornerPos(1) + size(img, 1), curCornerPos(2), sizeROI, sizeROI], 'EdgeColor', 'm', ...
                            'LineStyle', '-', 'LineWidth', 2);
                        pause(.5);
                    end

                    SWResult{windowIndex} = {[img(curVerticalIndx, curHorizantalIndx, :), ...
                        img(curVerticalIndx, curHorizantalIndx + size(img, 1), :)], ...
                        [curxPosCenter, curyPosCenter]};
        ...
        function cExpRecov = pupilRecov(obj, SMLM_img, backghat, brhat, varargin)
            %pupilRecov estimates the pupil phase  via
            %Zernike mode expansion (it returns
            %a set of coefficients each corresponding to a distinct mode).
            ...
            %                                   default: 30
            %............................................................
            %                                   x_channel: true- specifies
            %                                   x_channel calibration
            %                                   (default-false)
            %............................................................
            %                                   y_channel: true- specifies
            %                                   y_channel calibration
            %                                   (default-true)
            %............................................................
            %------->-
            %output
            %------->-
            %                                   recovOutput structure with
            %                                   fields:
            %                                   cExpRecov: fitted
            %                                   Zernike coefficients.
            %............................................................
            %                                   backgBorder: number of border pixels for
            %                                   background estimation.
            %............................................................
            %                                   folder_path: calibration beads
            %                                   data folder path.
            %............................................................
            %                                   offset_folder_path: offset
            %                                   data folder path.
            %............................................................
            %                                   SMLM_img: camera images
            %                                   coresponding to ROI being
            %                                   analyzed (after offset
            %                                   subtraction and
            %                                   photoelectron conversion)
            %............................................................
            %                                   inputOption: structure-
            %                                   input options
            %............................................................
            %                                   pupilRecovPara: structure
            %                                   with fields:
            %
            %                                   pupilRecovPara.sizeROI:
            %                                   size of ROI to be analyzed.
            %%............................................................
            %                                   pupilRecovPara.centerROI:
            %                                   center of ROI to be
            %                                   analyzed.
            %%............................................................
            %                                   pupilRecovPara.maxIndZernikeMode:
            %                                   max index of Zernike modes (Noll index)

            %put input options into a structure s
            s = opt2struct(varargin);


            if isfield(s, 'gausssmoothsigmapixel')
                gsmoothpixel = s.gausssmoothsigmapixel;
            else
                gsmoothpixel = 0;
            end


            backghat = double(backghat);
            brhat = double(brhat);
            SMLM_img = double(SMLM_img);

            %prepare for pupil recovery
            %----------------------------------------------------------

            %get the maximum Noll index
            maxIndxZernikeMode = obj.pupilRecovPara.maxIndxZernikeMode;

            %calculate the number of modes given the maximum Noll index
            numZernikeMode = (maxIndxZernikeMode + 1) * (maxIndxZernikeMode + 2) / 2 - 1; %excluding the first term


            %set maximum number of iterations for maximum likelihood
            if isfield(s, 'maxiter')
                maxiter = s.maxiter;
            else
                maxiter = 50; %default
            end
            ...
            B = ones(size(SMLM_img)) .* backghat; % background


            %specify which measurement channel to be analyzed
            if isfield(s, 'x_channel') && s.x_channel


                pmask_x = obj.phaseMask(:, :, 1);


                c0 = zeros(numZernikeMode, 1); % initial coefficents

                %create an objective handle
                objective_h = @(c, EXXxBasis, EYYxBasis, EZZxBasis, phaseMask)neg_log_like_Pois_abrr(c, EXXxBasis, ...
                    EYYxBasis, EZZxBasis, obj.ZernikeBases, brhat, SMLM_img, B, phaseMask, ...
                    'xchannel', true, 'GaussSmoothSigmaPixel', gsmoothpixel);

                Optfunc = @(c)objective_h(c, obj.EXXxBasis, obj.EYYxBasis, ...
                    obj.EZZxBasis, pmask_x);

                cExpRecov = fminunc(Optfunc, c0, options);


                %                     %plot the recovered PSF at focus
                %                     h=figure;
                %                     subplot(1,2,1)
                %                     imagesc(fliplr(SMLM_img(:,:,1,AxialScanRnage(ceil(numel(AxialScanRnage)/2)))))
                %                     title('Experimental image (at focus)')
                %                     axis image
                %                     Emitter.position_para.x=0;
                %                     Emitter.position_para.y=0;
                %                     Emitter.position_para.z=obj.stage_zpositions(AxialScanRnage(ceil(numel(AxialScanRnage)/2)));
                %
                %                     [imgx]=obj.formImgIsotropic(Emitter,'ZernikeCoefficient',...
                %                         cExpRecov(:,AxialScanRnage(ceil(numel(AxialScanRnage)/2))),...
                %                         'imgSize',sizeROI);
                %
                %                     subplot(1,2,2)
                %                     imagesc(imgx)
                %                     title('Recovered image (at focus)')
                %                     axis image

            else

                pmask_y = obj.phaseMask(:, :, 2);
                c0 = zeros(numZernikeMode, 1); % initial coefficents

                %create an objective handle
                objective_h = @(c, EXXyBasis, EYYyBasis, EZZyBasis, phaseMask)neg_log_like_Pois_abrr(c, EXXyBasis, ...
                    EYYyBasis, EZZyBasis, obj.ZernikeBases, brhat, SMLM_img, B, phaseMask, ...
                    'ychannel', true, 'GaussSmoothSigmaPixel', gsmoothpixel);

                Optfunc = @(c)objective_h(c, obj.EXXyBasis, ...
                    obj.EYYyBasis, obj.EZZyBasis, pmask_y);
                ...
                %                     Emitter.position_para.z=obj.stage_zpositions(AxialScanRnage(ceil(numel(AxialScanRnage)/2)));
                %
                %                     [~,imgy]=obj.formImgIsotropic(Emitter,'ZernikeCoefficient',...
                %                         cExpRecov(:,AxialScanRnage(ceil(numel(AxialScanRnage)/2))),...
                %                         'imgSize',sizeROI);
                %
                %                     subplot(1,2,2)
                %                     imagesc(imgy)
                %                     title('Recovered PSF (at focus)')
                %                     axis image
            end
